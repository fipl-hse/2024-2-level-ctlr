...
from admin_utils.website_validation.constants import (
    BUTTONS_PATH,
    CheckStatuses,
    CONFIG_PATH,
    DIST_PATH,
...
from admin_utils.website_validation.utils import (
    create_driver,
...
def get_requests_required_headers(url: str, config: Config) -> list:
    ...
    for headers_attempt in HEADERS_ATTEMPTS:
        try:
            ...
            tmp_config._headers = {
                key: value for key, value in config._headers.items() if key in headers_attempt
            }
...
def is_site_dynamic(url: str, config: Config, headless: bool = True) -> SiteCheckStatus:
    ...
    try:
        ...
        logger.info(f"\t\tplain GET: {response.status_code}")
    ...
    driver = create_driver(config, headers=headers, is_headless=headless, timeout=60)
    ...
    driver.implicitly_wait(5)
    logger.info("\t\tready for links check")
...
def is_dynamic_with_buttons(url: str, driver: WebDriver) -> SiteCheckStatus:
    ...
    logger.info(f"\t\tnumber of show more links: {len(show_more_links)}")
...
def get_selenium_required_headers(url: str, config: Config, page_load_timeout: int = 60) -> list:
    ...
    for headers_attempt in HEADERS_ATTEMPTS:
        driver = create_driver(
            config, headers=headers_attempt, is_headless=True, timeout=page_load_timeout
        )
        try:
            driver.get(url)
            return headers_attempt
...
def get_links_with_one_of_names(driver: WebDriver, names: list) -> list[WebElement]:
    ...
    def validate_element(element: WebElement, search_name: str) -> bool:
        if (
            search_name.lower().strip() not in element.text.lower()
            or element.location["y"] < 20
    ...
    for name in all_possible_names:
        try:
            ...
            for element in [
                *driver.find_elements(By.XPATH, f"//button[contains(.,'{name}')]"),
                *driver.find_elements(By.XPATH, f"//a[contains(.,'{name}')]"),
            ]:
                if validate_element(element, name):
                    result_elements.append(element)
                    logger.info(f"\t\t\tadded button {name} with text {element.text[:100]}...")
    ...
    for name in all_possible_names:
        try:
            ...
            for element in [*driver.find_elements(By.XPATH, f"//div[contains(.,'{name}')]")]:
                if validate_element(element, name):
                    result_elements.append(element)
                    logger.info(f"\t\t\tadded button {name} with text {element.text}")
        except (NoSuchElementException, TimeoutException, WebDriverException):
            pass
...
def validate_websites(urls: list, config: Config) -> None:
    ...
    for url in urls:
        ...
        logger.info(f"\tsite check: {dynamic_check_result.result}")
        if dynamic_check_result.msg:
            logger.info(f"\tsite check msg: {dynamic_check_result.msg}")

        selenium_headers = []
        if dynamic_check_result.result == CheckStatuses.DYNAMIC:
            selenium_headers = get_selenium_required_headers(url, config)
            logger.info(f"\tnumber of selenium headers: {len(selenium_headers)}")

        requests_headers = []
        if dynamic_check_result.result != CheckStatuses.ERROR:
            requests_headers = get_requests_required_headers(url, config)
        logger.info(f"\tnumber of requests headers: {len(requests_headers)}")

        responses[url] = SiteCheckResult(
            requests_headers, selenium_headers, status=dynamic_check_result
        )
