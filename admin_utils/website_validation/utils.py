...
def define_blacklist_and_whitelist(site_results: dict[str, SiteCheckResult]) -> tuple:
    ...
    for site_result in site_results:
        if (
            site_results[site_result].status.result == CheckStatuses.ERROR
            or site_results[site_result].requests_headers == ["ERROR"]
            or site_results[site_result].selenium_headers == ["ERROR"]
        ):
...
def save_to_csv(site_results: dict[str, SiteCheckResult]) -> None:
    ...
    data_frame = pd.DataFrame(
        values, columns=["URL", "Type", "Notes", "Requests_headers", "Selenium_headers"]
    )
...
def create_driver(
    config: Config, headers: Union[list, tuple] = (), is_headless: bool = True, timeout: int = 30
) -> Chrome:
    """
